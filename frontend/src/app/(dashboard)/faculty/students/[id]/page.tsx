...
export default function FacultyStudent360Page({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    if (!token || !subjectId || !sectionId) return

    fetch(`http://localhost:5000/api/faculty/subjects/${subjectId}/sections/${sectionId}/students`, {
    ...
    .catch(console.error)
  }, [token, params.id, subjectId, sectionId])
      ...
      <Card className="glass w-full overflow-hidden bg-card shadow-xl border-white/10">
        <CardHeader className="bg-card/80 backdrop-blur-xl border-b border-white/5 py-8">
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="w-24 h-24 rounded-full bg-primary/20 text-primary flex items-center justify-center font-bold text-4xl shadow-inner">
